            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-hover">
                        <thead class="table-light"><tr><th>Icon</th><th>Nama</th><th>Slug</th><th>Produk</th><th>Aksi</th></tr></thead>
...
<div class="modal fade" id="catModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title" id="catModalTitle">Tambah Kategori</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
...
function showForm() { $('#catModalTitle').text('Tambah Kategori'); $('#catForm')[0].reset(); $('#catId').val(''); new bootstrap.Modal($('#catModal')).show(); }

function editCat(id) {
    $.get('<?= base_url('admin/categories/get') ?>/' + id, function(res) {
        if (res.status) {
            $('#catModalTitle').text('Edit Kategori'); $('#catId').val(res.data.id);
...
$('#catForm').on('submit', function(e) {
    ...
    $.ajax({ url: url, method: 'POST', data: new FormData(this), processData: false, contentType: false,
        success: function(res) { if (res.status) { showToast(res.message, 'success'); bootstrap.Modal.getInstance($('#catModal')[0]).hide(); loadCategories(); } else showToast(res.message, 'danger'); }
    });
});

function deleteCat(id) {
    Swal.fire({ title: 'Hapus kategori?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Ya', cancelButtonText: 'Batal'
    }).then(r => { if (r.isConfirmed) { $.post('<?= base_url('admin/categories/delete') ?>/' + id, function(res) {
        if (res.status) { showToast('Kategori dihapus', 'success'); loadCategories(); } else showToast(res.message, 'danger'); }); }});
}
