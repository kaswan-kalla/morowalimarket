                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        <thead class="table-light">
                            <tr><th>Kode</th><th>Tipe</th><th>Nilai</th><th>Min. Belanja</th><th>Kuota</th><th>Expired</th><th>Status</th><th>Aksi</th></tr>
                        </thead>
...
<div class="modal fade" id="voucherModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title" id="voucherModalTitle">Tambah Voucher</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <form id="voucherForm">
                <div class="modal-body">
                    ...
                    <div class="mb-3"><label class="form-label">Tipe Diskon</label>
                        <select name="discount_type" class="form-select" required>
                            <option value="percentage">Persentase (%)</option><option value="fixed">Nominal Tetap (Rp)</option>
                        </select></div>
...
function showForm() { $('#voucherModalTitle').text('Tambah Voucher'); $('#voucherForm')[0].reset(); $('#voucherId').val(''); new bootstrap.Modal($('#voucherModal')).show(); }

function editVoucher(id) {
    $.get('<?= base_url('admin/vouchers/get') ?>/' + id, function(res) {
        if (res.status) {
            let d = res.data;
            $('#voucherModalTitle').text('Edit Voucher'); $('#voucherId').val(d.id);
            $('input[name="code"]').val(d.code); $('select[name="discount_type"]').val(d.discount_type);
            $('input[name="discount_value"]').val(d.discount_value); $('input[name="min_purchase"]').val(d.min_purchase);
            $('input[name="max_usage"]').val(d.max_usage); $('input[name="expired_at"]').val(d.expired_at);
...
$('#voucherForm').on('submit', function(e) {
    ...
    $.post(url, $(this).serialize(), function(res) {
        if (res.status) { showToast(res.message, 'success'); bootstrap.Modal.getInstance($('#voucherModal')[0]).hide(); loadVouchers(); }
        else showToast(res.message, 'danger');
    });
});

function deleteVoucher(id) {
    Swal.fire({ title: 'Hapus voucher?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Ya', cancelButtonText: 'Batal'
    }).then(r => { if (r.isConfirmed) { $.post('<?= base_url('admin/vouchers/delete') ?>/' + id, function(res) {
        if (res.status) { showToast('Voucher dihapus', 'success'); loadVouchers(); } else showToast(res.message, 'danger'); }); }});
}
